...
Number::Number(int base, int decVal) : base(base) {
	fromDecInt(decVal);
}
...
Number Number::operator+ (const Number& other) const {
	if (base != other.base) return NULL;
...
Number Number::operator/ (const Number& other) const {
	if (base != other.base) return NULL;
	if (other.length == 1 && other.table[0] == 0) return NULL;
	...
	// Bie¿¹ca wartoœæ dzielonego elementu
	Number current(base);
	...
	for (int i = dividend.length - 1; i >= 0; i--) {
		// Dodawanie bie¿¹cej cyfry dzielnej do bie¿¹cej wartoœci
		...
		// Znajdywanie najwiêkszej cyfry x  (x * divisor <= current)
		int x = 0;
