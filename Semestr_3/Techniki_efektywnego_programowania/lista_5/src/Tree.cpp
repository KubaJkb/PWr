...
Result<void, Error> Tree::initTree(vector<string> args) {
    ...
    if (pushElement(DEFAULT_VALUE).isSuccess()) {
        ...
        delete root;
        root = nullptr;
        return Result<void, Error>::fail(new Error(Error::ErrorType::INVALID_ARGUMENT_COUNT, "Za malo argumentow aby stworzyc drzewo"));
    }
...
Result<void, Error> Tree::pushElement(string elem) {
    ...
    if (type == TYPE_NUMBER) {
        if (elem[0] == NEGATION) {
            elem.erase(0, 1);
            // ***************************** ERROR ********************************
            return Result<void, Error>::fail(new Error(Error::ErrorType::NEGATIVE_NUMBER, "Liczby ujemne " + elem + " nie sa dozwolone"));
        }
        ...
        if (elem != stripAlphanumeric(elem)) {
            // ***************************** ERROR ********************************
            return Result<void, Error>::fail(new Error(Error::ErrorType::INVALID_EXPRESSION, "W wyrazeniu " + elem + " znajduje sie niedozwolona wartosc"));
        }
        ...
        // ***************************** ERROR ********************************
        return Result<void, Error>::fail(new Error(Error::ErrorType::UNRECOGNIZED_ELEMENT, "Nierozpoznany typ elementu: " + elem));
    }
    ...
    if (root == nullptr) {
        ...
        if (!root->addChild(newNode)) {
            // ***************************** ERROR ********************************
            return Result<void, Error>::fail(new Error(Error::ErrorType::TREE_FULL, "Nie bylo miejsca w drzewie na dodanie wezla " + elem));
        }
...
Result<void, Error> Tree::printTree() {
    if (root == nullptr) {
        return Result<void, Error>::fail(new Error(Error::ErrorType::TREE_EMPTY, "Nie mozna wydrukowac pustego drzewa"));
    }
    root->printNodeIter();
    cout << "\n";
    return Result<void, Error>::ok();
...
Result<void, Error> Tree::printVars() {
    ...
    if (vars.empty()) {
        return Result<void, Error>::fail(new Error(Error::ErrorType::VARS_EMPTY, "W drzewie nie ma zadnych zmiennych"));
    }
...
Result<void, Error> Tree::computeTree(vector<string> args) {
    if (root == nullptr) {
        // ***************************** ERROR ********************************
        return Result<void, Error>::fail(new Error(Error::ErrorType::TREE_EMPTY, "Nie mozna obliczyc wartosci pustego drzewa"));
    }

    vector<string> vars = getVars();
    if (vars.size() != args.size()) {
        // ***************************** ERROR ********************************
        return Result<void, Error>::fail(new Error(Error::ErrorType::INVALID_ARGUMENT_COUNT, "Liczba podanych argumentow jest nieprawidlowa"));
    }
    ...
    for (int i = 0; i < args.size(); i++) {
        if (determineType(args[i]) == TYPE_NUMBER) {
            ...
            // ***************************** ERROR ********************************
            return Result<void, Error>::fail(new Error(Error::ErrorType::ARGUMENT_NOT_NUMBER, "Podany argument " + args[i] + " nie jest liczba"));
        }
    }
...
Result<void, Error> Tree::printLevels() {
    if (root == nullptr) {
        // ***************************** ERROR ********************************
        return Result<void, Error>::fail(new Error(Error::ErrorType::TREE_EMPTY, "Drzewo jest puste."));
    }
