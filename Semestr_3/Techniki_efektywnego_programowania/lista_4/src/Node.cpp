...
Result<double, Error> Node::computeNode(map<string, double> dict) {
    if (type == TYPE_NUMBER) {
        ...
        if (value == OPERATOR_ADD) { // "+"
            ...
            if (left.isSuccess() && right.isSuccess()) {
                if (right.getValue() == 0) {
                    return Result<double, Error>::fail(new Error("W trakcie obliczania wartosci drzewa wystapilo niedozwolone dzielenie przez 0"));
                }
                else if (right.getValue() < 0) {
                    return Result<double, Error>::fail(new Error("Nie mozna dzielic przez liczby ujemne"));
                }
                return Result<double, Error>::ok(left.getValue() / right.getValue());
            }
            else {
    ...
    // ***************************** ERROR ********************************
    return Result<double, Error>::fail(new Error("Nieznany blad podczas obliczania wartosci drzewa!"));
}
