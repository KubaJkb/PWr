...
public class QuickSortHoare<T> extends AbstractSwappingSortingAlgorithm<T> {
    private final PivotSelector<T> pivotSelector;

    public QuickSortHoare(Comparator<? super T> comparator, PivotSelector<T> pivotSelector) {
        super(comparator);
    ...
    private int partition(List<T> list, int low, int high) {
        ...
        int i = low + 1, j = high;
        do {
            while (i <= j && compare(list.get(i), pivot) <= 0) {
                i++;
            }
            //while (j > low && compare(list.get(j), pivot) >= 0) {
